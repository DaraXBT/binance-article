...
import { CHROME_CANDIDATES_FULL, findChromeExecutable, getDefaultProfileDir } from './binance-utils.js';
...
async function checkChrome(): Promise<void> {
  ...
  if (chromePath) {
    ...
    log('Chrome', false, 'Not found. Set BS_CHROME_PATH env var or install Chrome.');
  }
}
...
async function checkClipboardCopy(): Promise<void> {
  ...
  const tmpDir = await mkdtemp(path.join(os.tmpdir(), 'bs-check-'));
  try {
...
async function checkBun(): Promise<void> {
  if (process.versions.bun) {
    log('Bun runtime', true, `v${process.versions.bun}`);
    return;
  }
...
async function main(): Promise<void> {
  console.log('=== baoyu-post-to-binance-square: Permission & Environment Check ===\n');
  ...
  if (failed.length === 0) {
    console.log('All checks passed. Ready to post to Binance Square.');
  } else {
